...
// تفعيل إضافة الـ Stealth لمنع حظر البوتات وتجاوز حمايات الـ Cloudflare
// @ts-ignore
...
// دالة ديناميكية لتشغيل المتصفح حسب البيئة المتواجد فيها الكود
async function getBrowserInstance() {
  const isProduction = process.env.NODE_ENV === "production";

  if (isProduction) {
    // 1. تحميل خطوط إضافية لدعم النصوص العربية والرموز التعبيرية على سيرفر Vercel Linux
    await chromium.font("https://raw.githack.com/wiki/jaimecbernardo/GFontsSpace/fonts/NotoColorEmoji.ttf");

    // 2. تشغيل المتصفح عبر الحزمة المصغرة المتوافقة مع AWS Lambda المستضافة على GitHub
    ...
    // 3. التشغيل في البيئة المحلية (Local Development)
...
// CORS Proxy لحل مشاكل الـ Cross-Origin للأصول الخارجية
...
// المسار الرئيسي لإنتاج ملفات الـ PDF وتحويل المواقع
app.post("/api/pdf", async (req, res) => {
  ...
  try {
    ...
    // إعداد أبعاد الشاشة لضمان استجابة تصميم الموقع المطلوب سحبه
    await page.setViewport({ width: 1280, height: 800 });

    console.log(`Navigating to target URL: ${url}`);
    
    // التحكم في طريقة الانتظار لضمان تحميل أكواد الـ JavaScript في الصفحة الهدف
    if (waitMode === "manual") {
    ...
    // توليد ملف الـ PDF كـ Buffer لحفظه واستهلاكه مباشرة في الذاكرة السريعة دون حفظ ملفات على القرص
    ...
    // إغلاق المتصفح دائماً لمنع تسريب الذاكرة وتجمد وظيفة الـ Serverless على Vercel
    if (browser) {
      try {
        await browser.close();
      } catch (e) {
...
// إعداد خادم الويب والـ Vite Middlewares
const setupServer = async () => {
  if (process.env.NODE_ENV !== "production") {
    ...
    // إعدادات بيئة الإنتاج لقراءة ملفات الفرونت إند بعد عمل Build
